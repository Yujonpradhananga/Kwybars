...
pub fn for_each_particle(
    ...
    match orientation {
        BarOrientation::Horizontal => {
            ...
            let particle_diameter = (max_radius * 2.0 * scale).max(2.0);
            ...
            for (index, value) in values.iter().enumerate() {
                let x = start_x + (index as f64 * (particle_diameter + gap_width)) + (particle_diameter * 0.5);
                let y = height * 0.5;
        ...
        BarOrientation::Vertical => {
            ...
            let rendered_total = (count * particle_diameter) + ((count - 1.0).max(0.0) * gap_height);
            let start_y = (height - rendered_total).max(0.0) * 0.5;

            for (index, value) in values.iter().enumerate() {
                let x = width * 0.5;
                let y = start_y + (index as f64 * (particle_diameter + gap_height)) + (particle_diameter * 0.5);
                let radius = (particle_diameter * 0.5 * value.clamp(0.01, 1.0)).max(1.0);
                paint(index, ParticleSpec { x, y, radius });
            }
        }
